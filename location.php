                    ...
                    <div class="col align-items-stretch rounded">
                        <div class="card h-100">
                            ...
                            <div class="card-body">
                                ...
                                <?php if (!$checkLike) : ?>
                                    <form action="comment.php?id=<?php echo $userID ?>&pid=<?php echo $post['pID'] ?>&action=loc" method="post" class="form-control mb-2">
                                        <input type="text" name="content" class="form-control mb-2">
                                        <button type="submit" class="btn btn-primary" href="comment.php?id=<?php echo $userID ?>&pid=<?php echo $post['pID'] ?>&action=loc" role="button">Comment</button>
                                    </form>
                                    <a class="btn btn-primary" href="like.php?id=<?php echo $userID ?>&pid=<?php echo $post['pID'] ?>&action=like&loc" role="button">Like</a>
                                    <a class="btn btn-danger" href="report.php?id=<?php echo $userID ?>&pid=<?php echo $post['pID'] ?>" role="button&loc">Report</a>
                                <?php elseif ($checkLike) : ?>
                                    <form action="comment.php?id=<?php echo $userID ?>&pid=<?php echo $post['pID'] ?>&action=loc" method="post" class="form-control mb-2">
                                        <input type="text" name="content" class="form-control mb-2">
                                        <button type="submit" class="btn btn-primary" href="comment.php?id=<?php echo $userID ?>&pid=<?php echo $post['pID'] ?>&action=loc" role="button">Comment</button>
                                    </form>
                                    <a class="btn btn-warning" href="like.php?id=<?php echo $userID ?>&pid=<?php echo $post['pID'] ?>&action=dislike&loc" role="button">Dislike</a>
                                    <a class="btn btn-danger" href="report.php?id=<?php echo $userID ?>&pid=<?php echo $post['pID'] ?>" role="button&loc">Report</a>
                                <?php endif; ?>
        ...
        <nav aria-label="...">
            <ul class="pagination pagination-lg justify-content-center">
                ...
                ?>
                    ...
                    <?php else : ?>
                        <li class="page-item"><a class="page-link" href="location.php?id=<?php echo $sessionID ?>&locID=<?php echo $locID ?>&page=<?php echo ($i + 1) ?>"><?php echo ($i + 1) ?></a></li>
                    <?php endif; ?>
                <?php } ?>
            </ul>
        </nav>
